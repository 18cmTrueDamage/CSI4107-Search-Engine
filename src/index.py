...
COURSE_DICT = os.path.dirname(os.path.abspath(__file__)) + '/../courses.json'
...
class Index:
    ...
    def __init__(self, corpus=None, df_dict=None, docid_tf_dict=None, doc_ids=None):
        ...
        self.tf_idf_matrix = self._get_tf_idf_matrix(
            tf_matrix, df_matrix, n=self.doc_count)
    ...
    def _create_documents(corpus):
        ...
        with open(corpus, 'r') as corpus_file:
            ...
            for row in reader:
                if len(row) == 0:  # CSV file contains empty lines somehow
                    continue
                doc_id = row[0]
                title = row[1]
                content = row[2]
                ...
                # create a dictionary for corpus access
                doc = {}
                doc["title"] = title
                doc["content"] = content
        ...
        with open(COURSE_DICT, 'w') as course_dict_file:
            json_string = json.dumps(docs_dict, indent=4, sort_keys=True)
            course_dict_file.write(json_string)
